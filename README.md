...
Go 代理启动前，**项目根目录**下需存在以下配置文件（与 `proxy_manager` 同级）：

| 文件 | 必须 | 生成方式 |
|------|------|----------|
| `proxy_current.json` | 是 | 运行 `python get_proxy_info.py [line_sn]` 或 `python get_pac.py [line_sn]`，包含当前线路 `sid` 及认证 `username`/`password` |
| `lines.json` | 是（面板与切换依赖） | 运行 `python linelist.py` |
| `lines_proxy.json` | 是（转发上游依赖） | 运行 `python get_all_lines_proxy.py`，按线路写入各线路上游 proxy 串 |

缺一不可：少任一文件或当前 `sid` 不在 `lines_proxy.json` 中，代理会报错或无法转发。首次使用请按 [使用流程](#使用流程) 依次执行 Python 脚本生成上述文件。

**编译 Go 代理**（可选，用于生成可执行文件）：
...
生成的 `proxy_manager.exe` 仍须在 `proxy_manager` 目录下运行，以便从父目录读取配置。

**从 GitHub Releases 下载**：若不想自行编译，可前往 [Releases](https://github.com/PF-UI/ilink-proxy-utils/releases) 下载对应平台的 `proxy_manager.exe`，解压后按上述方式放置并准备配置文件即可。
...
1. **编译**：`cd proxy_manager` → `go build -o proxy_manager.exe .`
2. **打标签并推送**：`git tag v1.0.0`（或新版本号）→ `git push origin v1.0.0`
3. **创建 Release**：打开仓库 **Releases** → **Create a new release** → 选择刚推送的标签（如 `v1.0.0`），填写标题与说明（可复制 `RELEASE_NOTES.md` 内容，说明需要的配置文件与运行方式），在 **Attach binaries** 中上传 `proxy_manager/proxy_manager.exe`，发布即可。
